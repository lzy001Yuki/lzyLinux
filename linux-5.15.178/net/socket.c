...
SYSCALL_DEFINE3(configure_socket_fairness, pid_t, thread_id, 
                int, max_socket_allowed, int, priority_level)
{
    ...
    rcu_read_lock();
    task = find_task_by_vpid(thread_id);
	pr_info("TID:%d\n", thread_id);
    if (task) {
        get_task_struct(task);
        rcu_read_unlock();
        
        task->max_sockets = max_socket_allowed;
        task->priority_level = priority_level;
        pr_info("successfully configure\n");
        put_task_struct(task);
        ret = 0;
    } else {
        rcu_read_unlock();
		pr_info("no configure\n");
    }
...
{
	if (current->max_sockets != -1) {
        // 使用原子读，避免竞争
        if (atomic_read(&current->socket_count) >= current->max_sockets) {
            pr_info("socket_fairness: Denying socket for TID %d. Limit: %d, Current: %d\n",
                    current->pid, current->max_sockets, atomic_read(&current->socket_count));
