...
)
{
   EFI_STATUS Status;
    ...
    // 使用分配内存的方式初始化 NewTable
    Status = gBS->AllocatePool(EfiRuntimeServicesData, sizeof(EFI_ACPI_DESCRIPTION_HEADER), (VOID**)&NewTable);
    if (EFI_ERROR(Status)) {
        Print(L"Failed to allocate memory for NewTable: %r\n", Status);
        return Status;
    }

    // 初始化 NewTable 的各个字段（模拟数据）
    NewTable->Signature = EFI_ACPI_6_3_FIXED_ACPI_DESCRIPTION_TABLE_SIGNATURE; // 示例签名
    NewTable->Length = sizeof(EFI_ACPI_DESCRIPTION_HEADER); // 假设修改表的长度与头大小一致
    NewTable->Checksum = 0;  // 假设初始校验和为 0
    NewTable->OemId[0] = 'Y';
    NewTable->OemId[1] = 'Y';
    NewTable->OemId[2] = 'Y';
    NewTable->OemId[3] = 'Y';
    ...
    // 调用修改 ACPI 表的函数
    Status = ChangeACPITable(EFI_ACPI_6_3_FIXED_ACPI_DESCRIPTION_TABLE_SIGNATURE, NewTable);
    if (EFI_ERROR(Status)) {
        Print(L"ChangeACPITable failed: %r\n", Status);
    ...
    Print(L"ACPI table modified successfully.\n");
